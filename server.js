// ===== BỘ NÃO CỦA HỆ THỐNG - PHIÊN BẢN TỔNG CHỈ HUY =====
// server.js
...
const fetch = require('node-fetch'); // Import node-fetch để gọi API khác
...
const OLD_APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzzTJWXLapiMmH6rTdaADD3wTLxm33aj3Cc3mfDQ5862wHXh2WrM2wZg9QMJXzhSFGQ/exec';
...
app.get('/api/staff-availability', async (req, res) => {
    // ... Phần này giữ nguyên như phiên bản trước, đã hoạt động tốt ...
    try {
...
// === API 2: Ghi nhận booking mới & CHỐNG TRÙNG LỊCH & TỰ ĐỘNG GỌI APPS SCRIPT CŨ ===
app.post('/api/bookings', async (req, res) => {
    try {
        ...
        // Kiểm tra trùng lịch chỉ với nhân viên được chọn
        if (preferredStaff) {
            const webBookingsRows = await webBookingsSheet.getRows();
            const walkinsRows = await walkinsSheet.getRows();
        ...
        // Tự động gọi đến Apps Script cũ để ghi vào sheet cũ
        ...
        formDataForOldScript.append('PreferredStaff', preferredStaff || 'Any');
        formDataForOldScript.append('PhoneNumber', phoneNumber);
        
        fetch(OLD_APPS_SCRIPT_URL, {
            method: 'POST',
            body: formDataForOldScript
        }).catch(err => console.error("Failed to call old Apps Script:", err)); // Bắt lỗi nhưng không chặn tiến trình

        res.json({ status: 'success', message: 'Booking processed.' });
