// ===== BỘ NÃO CỦA HỆ THỐNG - PHIÊN BẢN SỬA LỖI MÚI GIỜ =====
// server.js
...
app.get('/api/staff-availability', async (req, res) => {
  try {
    const { date } = req.query; // date có dạng "YYYY-MM-DD"
    ...
    if (!offDaysSheet || !webBookingsSheet || !walkinsSheet) {
        throw new Error("One or more required sheets (OffDays, WebBookings, Walkins) are missing.");
    }
    ...
    const nowInVietnam = new Date(now.getTime() - serverTimezoneOffset + vietnamTimezoneOffset);
...
app.post('/api/bookings', async (req, res) => {
    try {
        ...
        const staffBookingsOnDate = allBookings.filter(row => 
            ...
            row.get('booking_date') === bookingDate
        );

        let isConflict = false;
        for (const booking of staffBookingsOnDate) {
            const existingStartTime = new Date(`${booking.get('booking_date')}T${booking.get('start_time')}`);
            const existingEndTime = new Date(`${booking.get('booking_date')}T${booking.get('end_time')}`);
