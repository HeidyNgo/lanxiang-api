// ===== BỘ NÃO CỦA HỆ THỐNG - PHIÊN BẢN SỬA LỖI =====
// server.js

const express = require('express');
const cors = require('cors');
const { GoogleSpreadsheet } = require('google-spreadsheet');
const { JWT } = require('google-auth-library'); // Thư viện xác thực mới
...
// Hàm helper để truy cập Google Sheet (ĐÃ SỬA LỖI)
...
app.get('/api/staff-availability', async (req, res) => {
  try {
    ...
    const allStaffNames = [...new Set(offDaysRows.map(row => row.get('name')))];
    ...
    const availabilityResult = workingStaffNames.map(staffName => {
      ...
      if (staffBookings.length > 0) {
        // Tìm booking cuối cùng trong ngày
...
app.post('/api/bookings', async (req, res) => {
    try {
        ...
        let duration = 60; // Mặc định 60 phút nếu không tìm thấy
