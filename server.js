// ===== BỘ NÃO CỦA HỆ THỐNG - PHIÊN BẢN GỠ LỖI CUỐI CÙNG =====
// server.js
...
app.get('/api/staff-availability', async (req, res) => {
    // ... (Phần này đang hoạt động tốt, giữ nguyên)
    try {
...
// === API 2: Ghi nhận booking mới & CHỐNG TRÙNG LỊCH (PHIÊN BẢN CÓ GỠ LỖI) ===
app.post('/api/bookings', async (req, res) => {
    console.log('[DEBUG] Received new booking request.');
    try {
        ...
        console.log(`[DEBUG] Request for: ${preferredStaff} at ${bookingDateTime}`);
        ...
        console.log(`[DEBUG] New booking slot calculated: ${newStartTime.toISOString()} to ${newEndTime.toISOString()}`);
        ...
        console.log(`[DEBUG] Total bookings found in sheets: ${allBookings.length}`);
        ...
        console.log(`[DEBUG] Found ${staffBookingsOnDate.length} existing bookings for ${preferredStaff} on ${bookingDate}.`);
        ...
        for (const booking of staffBookingsOnDate) {
            ...
            const existingDateStr = normalizeDate(booking.get('booking_date'));
            
            console.log(`[DEBUG] Checking against existing booking: ${existingStartTimeStr} - ${existingEndTimeStr}`);

            if (existingStartTimeStr && existingEndTimeStr && existingDateStr) {
                const existingStartTime = new Date(`${existingDateStr}T${existingStartTimeStr}`);
                const existingEndTime = new Date(`${existingDateStr}T${existingEndTimeStr}`);
                
                console.log(`[DEBUG] Comparing new slot with existing: ${existingStartTime.toISOString()} - ${existingEndTime.toISOString()}`);
        ...
        if (isConflict) {
            console.log('[DEBUG] Rejecting booking due to conflict.');
            return res.status(409).json({ status: 'error', message: 'This time slot is already booked for the selected staff.' });
        }
        
        console.log('[DEBUG] No conflict found. Proceeding to save booking.');
        ...
        console.error('[ERROR] Error in /api/bookings:', error);
